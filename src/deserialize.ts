...
export function deserialize<T>(type: { new(): T }, source: any): T | null | undefined {
...
function getValue<T>(source: any, destination: T, propertyName: string, propertyMetadata: MessagePackMetadata) {
    let key = propertyMetadata.key ?? propertyName;
    let propertyType = getPropertyType(destination, propertyName);
    const formatterDeserialize = propertyMetadata.formatter ? propertyMetadata.formatter.deserialize : undefined;
